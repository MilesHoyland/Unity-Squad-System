...
public class SquadMemberAI : MonoBehaviour
{
    ...
    /// <summary>
    ///     If in follow state, go to desired pos. Recieve Movement Commands (Go To, Hold Position & Regroup).
    ///     If in follow state and Recieve Aggressive Push command Only start attacking when spotted.
    ...
    {
        Vector3 dir = (enemy.transform.position - bulletSpawnPoint.transform.position).normalized;
